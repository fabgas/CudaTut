...
public class Sum7 {
    ...
    static int n = 1<<24;
	static int blocks = getNumBlocks(n, 50000, 512);
...
//            durationCopy / 1e6, durationComp / 1e6);
    ...
    public static void prepare()
    {
        ...
        try
        {
            ptxFileName = preparePtxFile("sum7.cu");
        }
        catch (IOException e)
        {
    ...
    private static int getNumBlocks(int n, int maxBlocks, int maxThreads)
    {
        ...
        blocks = (n + (threads * 8 - 1)) / (threads * 8);
    ...
    {
        int threads = 0;
        threads = (n < maxThreads*8) ? nextPow2((n + 1)/ 8) : maxThreads;
        return threads;
    }
